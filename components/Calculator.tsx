    ...
    <div className="mx-auto max-w-5xl px-4 pb-16 pt-6">
      ...
      <div className="mb-5">
        ...
        <p className="mt-1 text-sm text-gray-500">
          ...
          <span className="font-medium text-gray-700">Skatteverkets allmänna tabeller 2026</span>{" "}
          (SKVFS 2025:20).
        </p>
      </div>
      ...
      <div className="mb-6 rounded-xl border border-gray-200 bg-gray-50 p-4">
        ...
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-4 lg:grid-cols-5">
          ...
          <div className="flex flex-col gap-0.5 min-w-0">
            ...
            <select
              value={taxTableNum}
              onChange={(e) => setTaxTableNum(Number(e.target.value))}
              className="rounded-lg border border-gray-300 bg-white px-3 py-2 text-sm text-gray-900 focus:border-blue-500 focus:ring-1 focus:ring-blue-500 focus:outline-none"
            >
              {AVAILABLE_TAX_TABLES.map((t) => (
                <option key={t.value} value={t.value}>{t.label}</option>
              ))}
            </select>
            <p className="text-xs text-gray-400 leading-tight">din kommunalskatt</p>
          </div>
        </div>
      </div>
